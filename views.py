...
def menu_view(request):
    ...
    return render(request, 'core/menu.html', {'user': user, 'settings': settings})
...
def dashboard_view(request):
    ...
    for label, qs in age_groups:
        total   = qs.count()
        with_d  = qs.filter(disability='Yes').count()
    ...
    sectors = [
        ('Labor Force',             residents.filter(is_labor_force='Yes').count()),
        ('Unemployed',              residents.filter(is_unemployed='Yes').count()),
        ('Out of School Children',  residents.filter(is_osc='Yes').count()),
        ('Out of School Youth',     residents.filter(is_osy='Yes').count()),
        ('Persons with Disability', residents.filter(disability='Yes').count()),
        ('Overseas Filipino Workers', residents.filter(is_ofw='Yes').count()),
        ('Solo Parents',            residents.filter(is_solo_parent='Yes').count()),
        ('Indigenous People',       residents.filter(is_indigenous='Yes').count()),
    ]
...
def residents_view(request):
    ...
    return render(request, 'core/residents.html', context)
...
def reports_view(request):
    ...
    total_pop  = residents.count()
    total_hh   = households.count()
    disability = residents.filter(disability='Yes').count()
    infra      = (
        Facility.objects.filter(category='Building').count() +
        Facility.objects.filter(category='Facility').count()
    )
    ...
    male_with_d    = residents.filter(gender='Male', disability='Yes').count()
    male_without_d = residents.filter(gender='Male', disability='No').count()
    female_with_d  = residents.filter(gender='Female', disability='Yes').count()
    female_without_d = residents.filter(gender='Female', disability='No').count()
...
def settings_view(request):
    ...
    return render(request, 'core/settings_page.html', context)
